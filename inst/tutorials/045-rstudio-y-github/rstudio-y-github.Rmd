...
author: Tejas Mundhe (traducido por Xavier de Pedro)
tutorial:
  id: rstudio-y-github
output:
  learnr::tutorial:
...
```{r copy-code-chunk, child = system.file("child_documents/copy_button.Rmd", package = "aprendeR")}
```

```{r info-section, child = system.file("child_documents/info_section.Rmd", package = "aprendeR")}
```
...
Eso es todo lo que necesitas saber sobre el Terminal para este tutorial. En el futuro, podrías completar un tutorial por separado llamado "Terminal" dedicado explícitamente a este tema. Asegúrate de prestar especial atención a si los comandos dados deben ejecutarse en la *Consola* (de R) o en el *Terminal* (del sistema).

## Git y configuración de GitHub
### 
...
*GitHub* es un sitio web que ofrece como una unidad en línea para todos tus proyectos y código en R de forma gratuïta mientras sean públicos (disponer de proyectos privados cuesta dinero allí). Sigue siendo muy usado porque fue el sitio web que más se popularizó de soporte web para el uso de *git*, como software de gestión de versiones de código fuente de programas informáticos. Ahora ya existen otros sitios similares, incluso con más funcionalidades y ventajas, como *Gitlab* que también puedes usar en vez de Github si te conviene (en los servidores gratuitos de _gitlab.com_ para proyectos públicos y privados también, o en los *Gitlab* instalados en servidores coporativos locales de las intranets de muchas administraciones públicas, por ejemplo).  En el mundo profesional, lo que tienes en tu cuenta de GitHub o Gitlab, por ejemplo, es más importante que lo que tienes en tu currículum. Es una demostración comprobable de tus habilidades.
...
En este tutorial usaremos *Github* para aprender como funciona todo. Lo que aprenderás aquí será muy similar lo que necesites en *gitlab.com* o en el *Gitlab* de la intranet tu institución, si es el caso.
...
Nombra tu repositorio `Primer_Repo`. Luego, selecciona la opción "public" para tu repositorio y marca la casilla que dice "Agregar un archivo README". [README](https://en.wikipedia.org/wiki/README) es un documento en el que las personas programadoras suelen añadir o explicar detalles de su proyecto. En caso de duda, sigue [estas instrucciones](https://happygitwithr.com/push-pull-github.html#make-a-repo-on-github).
...
La opción "public" significa que cualquiera podrá ver el repositorio mientras que solo tú puedes editarlo. Es posible que hayas notado "Add .gitignore" (Añadir .gitignore) como otra opción, pero nunca marcaremos esta opción porque RStudio creará automáticamente un archivo `.gitignore` para nosotros.
...
Ahora, has vinculado tu repositorio de GitHub a tu proyecto, pero no has demostrado que eres alguien con acceso de edición al proyecto. Si permitimos que cualquier persona con el enlace de GitHub lo edite, eso obviamente generará problemas de seguridad en el futuro. Esta es la razón por la que usamos algo llamado **T**oken de **A**cceso **P**ersonal, o **PAT** (de sus siglas en inglés).

###

Un PAT es solo una contraseña especial generada por Github para la comunicación entre tu ordenador personal (o el servicio en la nube donde estés ejecutando R) y GitHub. Esto permite a GitHub conectar tu cuenta de GitHub y el ordenador o servidor desde donde te conectas a Github (habitualmente, tu ordenador personal). Si deseas obtener más información, consulta la sección "[Token de acceso personal para HTTPS](https://happygitwithr.com/https-pat.html)" de [*Happy Git and GitHub for the useR*](https:// happygitwithr.com/)
...
```{r download-answers, child = system.file("child_documents/download_answers.Rmd", package = "aprendeR")}
```
